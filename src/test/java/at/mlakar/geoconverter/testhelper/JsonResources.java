package at.mlakar.geoconverter.testhelper;

public class JsonResources
{
	public static final String POINT_JSON = "src/test/resources/json/Point.json";
	public static final String POINT_NO_PROPERTIES_JSON = "src/test/resources/json/PointNoProperties.json";
	public static final String LINE_STRING_JSON = "src/test/resources/json/LineString.json";
	public static final String MULTI_LINE_STRING_JSON = "src/test/resources/json/MultiLineString.json";
	public static final String POLYGON_JSON = "src/test/resources/json/Polygon.json";
	public static final String MIXED_JSON = "src/test/resources/json/MixedElements.json";
}
